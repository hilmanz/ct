...
jQuery.imgAreaSelect.init=function(img,options)
{
    ...
    function getZIndex()
    {
        $p=$img;while($p.length&&!$p.is('body')){
    ...
    function adjust(){
        ...
        if(jQuery(parent).is('body'))parOfs=parScroll={
            ...
            parOfs=jQuery(parent).offset();parScroll={
                left:parent.scrollLeft,top:parent.scrollTop
            }
        }
        left=viewX(0);top=viewY(0)}
    function update(resetKeyPress){
        $a.css({left:viewX(selection.x1)+'px',top:viewY(selection.y1)+'px',width:Math.max(selection.width-options.borderWidth*2,0)+'px',height:Math.max(selection.height-options.borderWidth*2,0)+'px'});$areaOver.css({width:selection.width+'px',height:selection.height+'px'});
